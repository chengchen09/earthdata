...
void write_nc_header(int grp_id) {
	...
	for(i = 0; i < nvars; i++) {
		...
		status = nc_def_var(grp_id, var_list[i].name, vdat_list[i].type, vdat_list[i].ndims, var_dimids, &varid);
		NC_ASSERT(status);
...
int main(int argc, char **argv) {
	...
	int j;
	for(j = 0; j < 4; j++)
		  printf("%f\n", vdat_list[0].dbl_data[j]);
	for(j = 0; j < 4; j++)
		  printf("%f\n", vdat_list[1].dbl_data[j]);
	for(j = 0; j < 16; j++)
		  printf("%d\n", vdat_list[2].int_data[j]);
	
	//TODO: write_nc_data(ncfile_id);
